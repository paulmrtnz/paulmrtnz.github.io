...
**Content files** (YAML under `_data/{lang}/`, split per language — `_data/fr/` and `_data/en/`): Accessed in templates via `{% assign data = site.data[page.lang] %}`, then `data.projects`, `data.profile`. Keep the two language directories in sync manually — nothing enforces parity (has broken before: differing `featured` flags, extra/commented-out projects between languages). `data.profile` (`name`, `hook`, `description`) drives the landing-page hero in `_includes/landing_page.html`. `_data/photos.yml` (not language-split) feeds the photo gallery — a flat list of `{ id, alt }` where `id` is a Cloudinary public ID (photos are hosted on Cloudinary, not committed to the repo), consumed by `_includes/photo_page.html`.

**Photos (Cloudinary)**: images are not stored in the repo, to keep it small and because the site owner keeps full ownership of photos under Cloudinary's terms. Upload manually via the Cloudinary web dashboard, then add a `{ id, alt }` entry to `_data/photos.yml` by hand — `id` is the Cloudinary public ID (folder/filename, no extension). `_includes/photo_page.html` builds the delivery URL from `site.cloudinary_cloud_name` (`_config.yml`, a public value — not a secret, it's visible in every delivery URL) plus a fixed `f_auto,q_auto,w_1200,c_limit` transform (auto format/quality, no responsive `srcset` — keep it simple). No API keys, secrets, or `.env` file are used or needed for this — don't propose building an upload script/API integration unless asked.
